...
fourPeaksDF = pd.read_csv(os.path.join('data', 'SA_fourpeaks.csv'))
...
fig.savefig(os.path.join('image', 'SA_fourpeaks_iter.png'))

#fitness v. fevals
plt.clf()
sns_plot2=sns.lineplot(x="fevals", y="fitness", data=fourPeaksDF,)
fig2=sns_plot2.get_figure()
fig2.savefig(os.path.join('image', 'SA_fourpeaks_fevals.png'))
...
fig3.savefig(os.path.join('image', 'SA_fourpeaks_feval_iter.png'))