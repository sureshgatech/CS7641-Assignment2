...
df = pd.read_csv(os.path.join('data', 'SA_tsm.csv'))
...
fig.savefig(os.path.join('image', 'SA_tsm_iter.png'))

#fitness v. fevals
plt.clf()
sns_plot2=sns.lineplot(x="fevals", y="fitness", data=df,)
fig2=sns_plot2.get_figure()
fig2.savefig(os.path.join('image', 'SA_tsm_fevals.png'))
...
fig3.savefig(os.path.join('image', 'SA_tsm_feval_iter.png'))